...
7. **postship** — reconcile `count(packets)==count(outcomes)`, `graphify update .`, append `reference_*.md` + `MEMORY.md`, render `REPORT.md`, then **`devsync`** (`~/.claude/scripts/devsync.sh`) so local↔GitHub↔server end the run in sync — secret-scanned push at this checkpoint, never per-edit.

Every gate emits `{pass, reason, evidence}`. **No gate → no advance.** The two fan-out gates are strict and proven by `selftest/gates.test.mjs`: **recon** fails unless every expected scout returned a pack with real files (a missing scout is surfaced, not silently dropped); **verify** requires the full expected vote count + exact lens coverage + every vote `pass:true` with non-empty evidence (no "unanimous among survivors"). Parallel agents write per-agent event files (no shared-journal races). The engine tags failures with a taxonomy (`PARTIAL_QUORUM | LENS_MISSING | EVIDENCE_MISSING | VERDICT_FAIL`, `MISSING_SCOUT | EMPTY_PACK`). Contracts are **enforced**, not just specified: `scripts/validate.mjs` (AJV) checks docs against `schemas/*.json`; `scripts/run_state.mjs` is the canonical run-state with monotonic phase transitions + an epoch lock whose `save()` chokepoint rejects illegal/stale writes against on-disk truth; `scripts/quality.mjs` runs `.devflow/quality.yml`. Two selftests (`selftest/gates.test.mjs` 8/8, `selftest/contracts.test.mjs` 10/10) prove all of it and fail on regression.
