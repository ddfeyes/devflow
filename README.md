...
- **`/devsync`** (`scripts/devsync.sh`) does the full triangle for a repo: `fetch` → fast-forward/rebase local onto origin → secret-scan the outgoing diff (the exact commits being pushed — computed fail-*closed*, so a brand-new branch with no upstream is scanned too, not waved through) → push. It refuses on detached HEAD, mid-rebase, or a dirty divergence, and never leaves the repo half-synced. Generic secrets (private keys, AWS/GitHub/Slack/Google tokens, master-key assignments, 64-hex blobs) are blocked on **any** remote out of the box; **your site identifiers** (server IPs, SSH hosts, emails, home paths) are only caught once you add them to `~/.claude/devsync.public-deny.txt` — on a fresh install that file is the empty template, so populate it before pushing site-specific repos public.
